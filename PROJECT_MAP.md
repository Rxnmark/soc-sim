...
| `back/simulation.py` | **SimulationManager** — асинхронний рушій симуляції атак. Три типи атак: `DDoS` (простоює обладнання), `Stealth` (фінансовий вплив $50k/5s), `Ransomware` (шифрування з 15s таймером та 30s відновленням). Топологічні залежності (gateway → IoT/Endpoint). Методи: `start()`, `stop()`, `apply_fix()`, `get_status()`. |
