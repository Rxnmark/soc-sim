...
| `back/attack_definitions.py` | Конфігурація атак: `SIMULATION_ATTACKS` (DDoS, Stealth, Ransomware з лог-подіями та ефектами), `CRITICAL_GATEWAY_IDS`, константи часу (escalation, stealth interval, delay ranges, reboot seconds). |
| `back/simulation_core.py` | **SimulationCore** — асинхронна логіка гри: `_game_loop()`, `_spawn_attack()`, `_schedule_ransomware_encryption()`, `_apply_stealth_financial_impact()`, `_update_topology_dependencies()`, `_random_delay()`, `_pick_attack_type()`, `_generate_unique_ip()`, `_all_equipment_down()`. |
| `back/simulation.py` | **SimulationManager** (наспадник SimulationCore) — управління життєвим циклом: `start()`, `stop()`, `apply_fix()` (non-blocking з `_recovery_equipment()` фоновим завданням), `get_status()`. Імпортує логіку з simulation_core.py. |
...
| `front/src/translations/uk.ts` | Українські переклади UI елементів. |
| `front/src/translations/en.ts` | Англійські переклади UI елементів. |
...
| `cyber-threats.tsx` | Placeholder-сторінка для моніторингу кіберзагроз. |
...
| `expert-utils.tsx` | **Утиліти для експертної системи** — `LogStyle` тип, `getLogStyle()` для кольору подій (red/yellow/emerald), `translateLogEventType()` для перекладу типу події, `getEventDescription()` для отримання опису, `mapEventTypeToKey()` маппінг на translation key, `formatDate()` форматування часу. |
