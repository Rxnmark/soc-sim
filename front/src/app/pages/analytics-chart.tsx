...
interface Props {
    ddosValue: number;
    ransomwareValue: number;
    stealthValue: number;
    chartHistory: ChartPoint[];
    setChartHistory: (fn: (prev: ChartPoint[]) => ChartPoint[]) => void;
}

export function AnalyticsChart({ ddosValue, ransomwareValue, stealthValue, chartHistory, setChartHistory }: Props) {
    ...
    const yMax = useMemo(() => {
        ...
        const maxVal = Math.max(
            ...chartData.flatMap(d => [d.ddos, d.ransomware, d.stealth]),
        );
        ...
        <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
                ...
                <Line
                    type="monotone"
                    dataKey="ddos"
                    name={t('financialStats.ddos', 'DDoS')}
                    stroke="#ef4444"
                    strokeWidth={2}
                    dot={false}
                />
                <Line
                    type="monotone"
                    dataKey="ransomware"
                    name={t('financialStats.ransomware', 'Ransomware')}
                    stroke="#f97316"
                    strokeWidth={2}
                    dot={false}
                />
                <Line
                    type="monotone"
                    dataKey="stealth"
                    name={t('financialStats.stealth', 'Stealth / APT')}
                    stroke="#a855f7"
                    strokeWidth={2}
                    dot={false}
                />
