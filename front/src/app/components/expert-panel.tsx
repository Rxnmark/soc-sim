...
import { getCardClass, getCriticalityRank, isResolvedThreat, formatDate, translateLogEventType, getEventDescription, getLogStyle } from "./expert-utils";
...
export function ExpertPanel({ filterIp }: Props) {
  ...
  const displayedLogs = useMemo(() => {
    ...
    filtered = filtered.filter(log => !isResolvedThreat(log.event_type) && !archivedThreats.has(log.source_ip));
  ...
  const handleArchiveThreat = async (log: SecurityLog) => {
    ...
    try {
      await fetch("http://127.0.0.1:8000/api/v1/threats/archive", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ source_ip: log.source_ip }) });
      await fetch("http://127.0.0.1:8000/api/v1/actions/block", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ source_ip: log.source_ip }) });
  ...
  const handleApplyFix = async (log: SecurityLog) => {
    ...
    } catch (error) { console.error("Error applying fix:", error); setFixMessage("Error applying fix"); setTimeout(() => setFixMessage(null), 3000); }
  ...
  if (selectedLog) {
    ...
    return (
      <Card className="flex flex-col h-full w-full bg-card border border-border rounded-lg shadow-sm overflow-y-auto custom-scrollbar p-6">
        <Button variant="ghost" size="sm" className="w-fit mb-4 -ml-2 text-muted-foreground hover:text-foreground" onClick={() => setSelectedLog(null)}>
        ...
        <div className="space-y-5 mb-8 flex-1">
          <div>
            ...
            <div className={`p-4 rounded-lg border ${isResolved ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'}`}>
              <p className={`text-sm font-semibold mb-1.5 ${style.color}`}>{getEventDescription(t, selectedLog.event_type)}</p>
            </div>
          </div>
          <div><h3 className="text-sm font-semibold text-card-foreground mb-2.5">{t('logs.system_context', 'System Context')}</h3>
            <div className="space-y-2.5 rounded-lg border border-border p-3.5 bg-muted/20">
              <div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">{t('logs.source_ip', 'Source IP')}</span><code className="text-foreground bg-muted/60 px-2 py-0.5 rounded font-mono text-xs">{selectedLog.source_ip}</code></div>
              <div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">{t('logs.timestamp', 'Timestamp')}</span><span className="text-foreground font-medium">{formatDate(selectedLog.timestamp)} ({new Date(selectedLog.timestamp).toLocaleDateString()})</span></div>
            </div>
          </div>
          {isResolved ? (
              ...
              <div className="p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                <div className="flex items-start gap-3.5 mb-2"><CheckCircle2 className="w-5 h-5 text-emerald-500 mt-0.5 shrink-0" />
                  <div><p className="text-sm font-semibold text-emerald-500">{t('logs.threat_neutralized', 'Threat Successfully Neutralized')}</p>
                    <p className="text-sm text-muted-foreground mt-1.5 leading-relaxed">{t('logs.neutralized_desc', '')} {selectedLog.source_ip}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="p-4 rounded-lg bg-muted/40 border border-border">
                <div className="flex items-start gap-3.5 mb-2"><ShieldAlert className="w-5 h-5 text-yellow-500 mt-0.5 shrink-0" />
                  <div><p className="text-sm font-semibold text-foreground">{t('logs.block_ip_recommendation', 'Block Source IP via Firewall')}</p>
                    <p className="text-sm text-muted-foreground mt-1.5 leading-relaxed">{t('logs.block_desc', '')} {selectedLog.source_ip}{t('logs.block_desc_end', '')}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-3 pt-5 border-t border-border mt-auto shrink-0">
          ...
          {isResolved ? (
            ...
            <Button onClick={() => handleApplyFix(selectedLog)} disabled={isFixing} className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-semibold transition-all">
          ...
          <Button variant="outline" className="w-full" onClick={() => setSelectedLog(null)}>{t('logs.close_details', 'Close Details')}</Button>
        </div>
      </Card>
    );
  }
